...
# 특성이 몇개이고 줄어든 특성이 몇개인지를 확인해보려고 실습을 하는 것이다.
digits = datasets.load_digits() # 8x8 크기의 손글씨 숫자 데이터 로드
# 특성 행렬을 표준화 처리 (StandardScale을 사용한다.)
features = StandardScaler().fit_transform(digits.data)
# print(features)

# 99%의 분산을 유지하도록 PCA 클래스 객체 생성
# 이거는 잘 쓰지 않고 이런게 있다라는 정도만 알아두라고 말씀하심.
...
# 원본 특성 개수 : 64
# 줄어든 특성 개수 : 54
# 64개에서 54개로 특성이 줄어들었다는 것을 확인해 볼 수 있다.

# 주성분에서 투영된 처음 두 개의 특성을 사용해 산점도 출력
...
# 표준화하지 않은 원본 데이터 사용
pca.fit(digits.data) # 원본 데이터를 사용한다.
plt.plot(np.cumsum(pca.explained_variance_ratio_))
plt.show()
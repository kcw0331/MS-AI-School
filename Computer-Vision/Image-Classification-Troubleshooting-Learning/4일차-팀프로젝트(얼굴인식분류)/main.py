...
def main():
    ...
    train_dataset = customDataset("D:\\dataset\\train\\", transform = train_transform)
    val_dataset = customDataset("D:\\dataset\\val\\", transform = val_transform)
    # test_dataset = customDataset("D:\\dataset\\test\\", transform = test_transform)

    ### def visualize_augmentations()
    def visulize_augmentations(dataset, idx=1500, samples=20, cols=5):
    ...
    net.fc = nn.Linear(in_features=512, out_features=12)
    # net.load_state_dict(torch.load("D:/best.pt"))
    net.to(device)
    ...
    optimizer = torch.optim.AdamW(net.parameters(), lr=0.001) # net의 파라메타를 넣어줘야 한다.
    epochs = 100
    ...
    if os.path.exists(save_path):
        best_val_acc = max(pd.read_csv("D:/modelAccuracy.csv")["Train_Accuracy"].tolist())
    ...
    for epoch in range(epochs):
        ...
        if epoch == epochs - 1:
            dfForAccuracy.to_csv("D:/modelAccuracy.csv", index=False)

    torch.save(net.state_dict(), "D:/last.pt")

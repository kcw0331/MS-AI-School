...
# json xml -> 커스텀 데이터셋 -> 첫번쨰 실습 코드 결과 제출
...
def visualize_bbox(image, bboxes, category_ids, category_id_to_name,
    ...
    # # cv2.putText(img, text =class_name, org = (x_min, y_min + 30), fontFace = cv2.FONT_HERSHEY_SIMPLEX)# 그림위에 글자를 세기는 것이다.
    # cv2.putText(img, text=class_name, org=(x_min, y_min+30), fontFace = cv2.FONT_HERSHEY_SCRIPT_COMPLEX,
...
transfor = A.Compose([
    ...
    A.MultiplicativeNoise(multiplier=0.5, p=1) # 화면을 어둡게 하는 노이즈를 만들어준다.
    # A.MultiplicativeNoise(multiplier= [0.5, 1.5], elementwise=True, p=1) # 노이즈를 줄 수 있다.

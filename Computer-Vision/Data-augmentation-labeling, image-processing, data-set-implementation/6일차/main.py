# conda install -c pytorch pytorch <- 이거를 설치하였다.
import torch.nn as nn
from collections import defaultdict # 사전의 기본값을 처리해준다.
import torch
import cv2
import os
import copy
...
class catvsdogDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label_temp = label_temp[2].split(".")[0]
        label = 0
        if "cat" == label_temp:
            label = 0
        elif "dog" == label_temp:
...
def calculate_accuracy(output, target):
    ...
    target = target == 1.0
    return torch.true_divide((target == output).sum(dim=0), output.size(0)).item() # item은 tensor로 된 거를 정수로 뽑아낸때 사용한다.
...
class MetricMonitor:
    ...
    def __str__(self): # string으로 반환하라는 것이다.
        return " | ".join(
            [
...
params = {
    "model": "resnet18",
    "device": "cpu",  # 본인 환경에 맞은 device 설정 ex ) cuda, cpu
    "lr": 0.001,
    "batch_size": 64,
    "num_workers": 2,
    "epoch": 10
...
# model loader
model = models.__dict__[params["model"]](pretrained=True) # pretrained을 true로하면 1000개로 학습을 하게 된다.
model.fc = nn.Linear(512, 2)
model = model.to(params["device"])
# print(model)
# (fc): Linear(in_features=512, out_features=2, bias=True) # output features가 2개가 되면 라벨이 2개가 된다는 것이다.
...
def save_model(model, save_dir, file_name='last.pt'):
    # save model
    os.makedir(save_dir, exit_ok = True)
    output_path = os.path.join(save_dir, file_name)
    # ./weights/last.pt
    if isinstance(model, nn.DataParallel):
        print("multi GPU activate") # 멀티 GPU일 때만 사용이 가능하다.
        torch.save(model.module.state_dict(), output_path)
    else:
        print("single GPU activate") # 싱글 GPU일 때 사용해준다.
